...
base_experiments_dir = "experiments" # Stąd CZYTAMY oryginalne wagi i logi DANN
...
# =================================================================
# CHECK WHAT'S COMPLETED (W NOWYM FOLDERZE PROBINGU)
# =================================================================
MY_TARGET_HOSPITALS = [
...
for folder_name in os.listdir(probing_experiments_dir):
    folder_path = os.path.join(probing_experiments_dir, folder_name)
    if os.path.isdir(folder_path):
        ...
        probing_results_file = os.path.join(folder_path, "final_results_frozen_alpha.csv")
        if os.path.exists(probing_results_file):
...
for TARGET_HOSPITAL_CODE in MY_TARGET_HOSPITALS:
    ...
    model_weights_path = os.path.join(source_dir, "best_model.pt")
    ...
    # Inicjalizacja nowych plików logowania
    ...
    # Odczytywanie logów DANN (informacyjnie z oryginalnego eksperymentu)
    ...
    test_loader = Loader(data_pth, test_pool, minet_subsampling_n=None, num_workers=2).get_batched_loader(batch_size=1, pad=True)
    ...
    probing_optimizer = optim.AdamW(dann_model.backbone.classifier.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for epoch in range(PROBING_EPOCHS):
        ...
        pbar = tqdm(range(STEPS), desc=f"Probing {epoch+1}/{PROBING_EPOCHS}", leave=False)
