...
# Filtrowanie globalne (Tylko ALLOWED)
...
for current_target in MY_TARGET_LIST:
    ...
    # Zostawiamy zapis do experiments_finetuning
    ...
    # 2. PRZYGOTOWANIE DANYCH (LOSO dla obecnego targetu)
    ...
    if not target_pool:
        print(f"⚠️ Pominiecie {current_target}: Brak danych (pusta lista Target Pool).")
        continue # Przejdź do następnego szpitala w pętli
...
# Loaders
    ...
    # Optimizer (Smart Freezing)
    backbone_params = []
    heads_params = []

    for name, param in dann_model.backbone.named_parameters():
        if "temporal" in name or "spatial" in name or "bnorm_1" in name:
            param.requires_grad = False
        elif "classifier" in name or "linear" in name:
    ...
    for param in dann_model.domain_classifier.parameters():
        param.requires_grad = True
        heads_params.append(param)

    optimizer = optim.Adam([
        {'params': backbone_params, 'lr': 1e-4}, 
        {'params': heads_params,    'lr': 1e-3}
    ])
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch < 2: alpha_target = 0.0
        else: alpha_target = 1.0

        pbar = tqdm(range(STEPS), desc=f"Epoka {epoch+1}/{EPOCHS}", leave=False)
        
        for i in pbar:
            if epoch >= 2:
                p = float(i + (epoch-2) * STEPS) / (EPOCHS * STEPS)
                alpha = (2. / (1. + np.exp(-10 * p)) - 1) * alpha_target
            else:
                alpha = 0.0
            ...
            c_pred_s, d_pred_s = dann_model(s_X, time_stamps=s_ts, alpha=alpha)
            loss_cls = loss_class_fn(c_pred_s, s_y)
            loss_dom_s = loss_domain_fn(d_pred_s, d_label_s)

            _, d_pred_t = dann_model(t_X, time_stamps=t_ts, alpha=alpha)
    ...
    # 1. Final Model
    run_evaluation(dann_model, test_loader_full, device, prefix="Final_")
