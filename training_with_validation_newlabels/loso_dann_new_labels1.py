...
EXP_ROOT = "experiments_dann_new_labels"
...
def clean_eid_str(eid_raw):
    """
    Wydobywa z ciągu znaków identyfikator UUID (to, co w nawiasach {}).
    """
...
def map_to_binary(val):
    ...
    if val in ['0', '0.0', 'normal', 'norma', 'norm', 'false']:
        ...
        return 1 # Fallback dla każdej innej niestandardowej nazwy patologii
...
if os.path.exists(new_csv_path):
    ...
    # RĘCZNY PARSER OMIJAJĄCY PANDASA
        ...
        if 'CLAS_SVC_1B' in header:
            ...
            for line in f:
                ...
                if eid_clean in uuid_to_label:
                    try:
                        new_clas_val = parts[-dist_from_right]
                    except IndexError:
                        continue
                    ...
                    if binary_val is not None:
                        ...
                        if binary_val == 0: norma_count += 1
                        else: patho_count += 1
                        
            print(f"   -> UDAŁO SIĘ! Aplikowano i nadpisano {new_labels_applied} etykiet z nowej tabeli.")
            print(f"      Z tego Norma: {norma_count}, Patologia: {patho_count}")
        else:
            print("   ⚠️ UWAGA: Nie znaleziono kolumny 'CLAS_SVC_1B' w nowym pliku CSV!")
else:
...
for TARGET_HOSPITAL_CODE in MY_TARGET_HOSPITALS:
    ...
    for epoch in range(EPOCHS):
        ...
        for i in pbar:
            ...
            pbar.set_postfix({'Total': f"{loss.item():.2f}", 'Cls': f"{loss_cls.item():.2f}", 'Dom': f"{loss_dom.item():.2f}", 'Alpha': f"{alpha:.2f}"})
        ...
        val_loss, val_auc = evaluate_loss_and_auc(dann_model, val_loader, device, loss_cls_fn)
        test_loss, test_auc = evaluate_loss_and_auc(dann_model, test_loader_epoch, device, loss_cls_fn)
        ...
        if val_auc > best_val_auc:
            ...
            best_model_wts = copy.deepcopy(dann_model.state_dict())
            torch.save(dann_model.state_dict(), MODEL_BEST_PATH) 

    print("\n Trening zakończony. Zapis modelu Final...")
    torch.save(dann_model.state_dict(), MODEL_FINAL_PATH)
    ...
    run_target_evaluation(dann_model, test_loader_final, device, prefix="Final_")
    evaluate_comprehensive(dann_model, train_loader, test_loader_final, device, max_batches=200, prefix="Final_")

    print(f"\n Przywracanie wag BEST (Val_AUC={best_val_auc:.4f})...")
    dann_model.load_state_dict(best_model_wts)
    run_target_evaluation(dann_model, test_loader_final, device, prefix="Best_")
    evaluate_comprehensive(dann_model, train_loader, test_loader_final, device, max_batches=200, prefix="Best_")

print("\n ZAKOŃCZONO ANALIZĘ WSZYSTKICH SZPITALI Z NOWYMI LABELAMI")